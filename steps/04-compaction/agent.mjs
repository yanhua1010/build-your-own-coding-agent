#!/usr/bin/env node
// Step 04：context compaction，让 agent 能持续工作。
// 对应系列文章第 4 篇。
//
// 和 step 03 的区别全在主循环的两处新增检查：
//   1. 每轮对话结束后估算 token 数，超过阈值就压缩
//   2. 模型返回 context overflow 错误时，压缩后自动重试一次
//
...
async function beforeToolCall({ call, args }) {
	...
	if (hit) return { block: true, reason: `命令被安全策略拦截（匹配 ${hit}）。请换一个不会造成不可逆破坏的做法。` };

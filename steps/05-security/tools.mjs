...
import { cleanEnv, wrapWithSandbox, killTree } from "./sandbox.mjs";

const BASH_THROTTLE_MS = 100;

// bash 的外部沙箱开关。agent.mjs 按 AGENT_SANDBOX 设置。
// 默认关 —— 对应文章：能力（wrapWithSandbox）在，默认不开。
export const bashConfig = { sandbox: false };
...
export const tools = {
	...
	bash: {
		description: "在当前目录执行 shell 命令，返回 stdout 和 stderr。执行前会清洗掉密钥类环境变量。",
		parameters: {
			type: "object",
			properties: {
				command: { type: "string" },
		...
		execute: ({ command, timeout = 60 }, { signal, onUpdate }) =>
			new Promise((resolveExec, rejectExec) => {
				// 加固三件事：可选套外部沙箱、独立进程组（便于整树 kill）、清洗 env。
				const { file, args } = wrapWithSandbox(command, {
					cwd: process.cwd(),
					enabled: bashConfig.sandbox,
				});
				...
				const killTimer = setTimeout(() => killTree(child, "SIGKILL"), timeout * 1000);
